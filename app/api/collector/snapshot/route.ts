...
export async function POST(request: Request) {
  try {
    ...
    const elements = Array.isArray(body.elements) ? body.elements.slice(0, 800).filter((item): item is CollectedElement => !!item && typeof item === "object" && typeof (item as CollectedElement).text === "string") : [];
