...
function IntegrationCenter({ staticHosting, figmaReady, collectorReady, canCollect, onFigma, onCollector, onUploadDesign, onUploadActual, onClose }: { staticHosting: boolean; figmaReady: boolean; collectorReady: boolean; canCollect: boolean; onFigma: () => void; onCollector: () => void; onUploadDesign: () => void; onUploadActual: () => void; onClose: () => void }) {
  return <div className="integration-backdrop" role="presentation" onMouseDown={onClose}><aside className="integration-panel" role="dialog" aria-modal="true" aria-labelledby="integration-title" onMouseDown={event => event.stopPropagation()}>
    <header><div><small>CONNECTION CENTER</small><h2 id="integration-title">连接与集成</h2><p>跟随步骤读取真实设计稿与开发页面数据。</p></div><IconButton label="关闭连接工具" onClick={onClose}>×</IconButton></header>
    ...
    <section className="integration-card"><div className="integration-card-head"><span className="integration-icon">F</span><div><b>Figma 设计稿</b><small>读取 Frame 图片、字体与设计属性</small></div><em className={figmaReady ? "is-ready" : ""}>{figmaReady ? "已读取" : staticHosting ? "需本地版" : "未连接"}</em></div>
      <details><summary>查看连接教程</summary><ol><li><span>1</span><p><b>准备本地配置</b>在项目根目录的 <code>.env.local</code> 中填写 Figma Client ID、Client Secret 和回调地址。</p></li><li><span>2</span><p><b>连接 Figma</b>点击下方按钮登录并允许 Inspector 读取你有权限访问的文件。</p></li><li><span>3</span><p><b>复制 Frame 链接</b>在 Figma 选中 Frame，复制链接后粘贴到导入窗口。</p></li><li><span>4</span><p><b>读取并展示</b>系统会导入 Frame 图片，并提取文字、字号、行高和尺寸。</p></li></ol></details>
    ...
    <section className="integration-card"><div className="integration-card-head"><span className="integration-icon">C</span><div><b>Chrome 页面采集</b><small>采集可见截图、文字样式与组件尺寸</small></div><em className={collectorReady ? "is-ready" : ""}>{collectorReady ? "已采集" : staticHosting ? "需本地版" : "未采集"}</em></div>
      <details><summary>查看安装与采集教程</summary><ol><li><span>1</span><p><b>加载扩展</b>打开 <code>chrome://extensions</code>，开启开发者模式并加载项目中的 <code>chrome-extension</code> 文件夹。</p></li><li><span>2</span><p><b>启动本地服务</b>运行项目后，确认 Inspector 使用 3000 端口、采集服务使用 3001 端口。</p></li><li><span>3</span><p><b>复制会话 ID</b>点击下方“开始页面采集”，复制系统生成的会话 ID。</p></li><li><span>4</span><p><b>采集当前网页</b>在待检查网页打开 TARS 扩展，粘贴 ID 并允许本地访问。</p></li><li><span>5</span><p><b>返回完成匹配</b>回到 Inspector 检查连接，截图会进入画布，CSS 属性会自动匹配。</p></li></ol></details>
      <Button variant="primary" disabled={staticHosting || !canCollect} title={!canCollect ? "请先从 Figma 读取一个 Frame" : undefined} onClick={onCollector}>{collectorReady ? "重新采集页面" : canCollect ? "开始页面采集" : "请先读取 Figma Frame"}</Button>
    </section>
...
export function InspectorWorkspace() {
  ...
  return <main className="app-shell">
    <header className="topbar">
      ...
      <Button className="integration-trigger" onClick={() => setIntegrationOpen(true)}>连接工具 <span>{Number(!!designInspections[pageId]) + Number(!!captureMeta[pageId])}/2</span></Button>
