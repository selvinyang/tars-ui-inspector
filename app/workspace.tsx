...
function IssueThumbnail({ src, label, regionWidth, regionHeight }: { src: string; label: string; regionWidth?: number; regionHeight?: number }) {
  // Local canvas output cannot use the hosted image optimizer.
  // eslint-disable-next-line @next/next/no-img-element
  const boxWidth = Math.max(18, Math.min(88, regionWidth ? regionWidth / 480 * 100 : 32));
  const boxHeight = Math.max(20, Math.min(88, regionHeight ? regionHeight / 300 * 100 : 38));
  return <div className="issue-thumbnail-wrap"><img className="issue-thumbnail" src={src} alt={label} /><span className="issue-region-outline" style={{ width: `${boxWidth}%`, height: `${boxHeight}%` }} aria-hidden="true"></span></div>;
}
...
export function InspectorWorkspace() {
  ...
  function createRegionIssue(region: DifferenceRegion) { const next: Issue = { id: Math.max(0, ...issues.map(issue => issue.id)) + 1, title: `自动识别差异区域 #${region.id}`, description: `当前表现：该区域与设计稿存在明显视觉差异。\n设计预期：请对照设计稿确认具体组件样式。\n检测信息：差异范围约 ${region.width} × ${region.height}px，区域内 ${region.changedPercent}% 像素发生变化。`, type: "与设计稿不一致", priority: "P2", pageId, deviceId, x: region.leftPercent + region.widthPercent / 2, y: region.topPercent + region.heightPercent / 2, canvasWidth: device.width, canvasHeight, regionWidth: region.width, regionHeight: region.height, createdAt: "刚刚" }; setEditing(next); setSelectedId(next.id); }
  ...
  return <main className="app-shell">
    ...
    <aside className="right-panel">
      {editing ? <IssueForm issue={editing} onSave={saveIssue} onCancel={() => setEditing(null)} /> : <>
        ...
        <div className="issue-list">{visibleIssues.map(issue => <article key={issue.id} className={selectedId === issue.id ? "is-active" : ""} onClick={() => selectIssue(issue)}><div className="issue-compare"><figure><figcaption>开发稿</figcaption>{(issue.actualThumbnail ?? issue.thumbnail) ? <IssueThumbnail src={(issue.actualThumbnail ?? issue.thumbnail)!} label="开发稿局部截图" regionWidth={issue.regionWidth} regionHeight={issue.regionHeight} /> : <div className="issue-shot-placeholder">暂无截图</div>}</figure><figure><figcaption>UI 设计稿</figcaption>{issue.designThumbnail ? <IssueThumbnail src={issue.designThumbnail} label="UI设计稿局部截图" regionWidth={issue.regionWidth} regionHeight={issue.regionHeight} /> : <div className="issue-shot-placeholder">暂无截图</div>}</figure></div><div className="issue-card-top"><span className={`priority-tag ${issue.priority}`}>{issue.priority}</span><b>#{issue.id}</b><time>{issue.createdAt}</time></div><h3>{issue.title}</h3><p className="issue-description">{issue.description || "暂无问题描述"}</p><p className="issue-meta">{issue.type} · {projectPages.find(p => p.id === issue.pageId)?.name}</p><div className="issue-card-bottom"><span>{devices.find(d => d.id === issue.deviceId)?.width} × {devices.find(d => d.id === issue.deviceId)?.height}</span><button onClick={e => { e.stopPropagation(); copyText(issueText(issue, projectPages), `问题 #${issue.id} 已复制`); }}>复制</button><button onClick={e => { e.stopPropagation(); setEditing(issue); }}>编辑</button><button onClick={e => { e.stopPropagation(); if (confirm(`删除问题 #${issue.id}？`)) setIssues(old => old.filter(i => i.id !== issue.id)); }}>删除</button></div></article>)}{visibleIssues.length === 0 && <div className="empty-state"><b>{issues.length ? "没有匹配的问题" : "尚未记录问题"}</b><span>{issues.length ? "调整筛选条件后再试" : "上传页面截图与设计稿后，在画布上点击创建标注"}</span></div>}</div>
      </>}
    </aside>
