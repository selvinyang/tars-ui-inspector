...
function CollectorDialog({ id, origin, loading, error, onCopy, onCheck, onClose }: { id: string; origin: string; loading: boolean; error: string; onCopy: (value: string) => void; onCheck: () => void; onClose: () => void }) {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}><div className="dialog collector-dialog" role="dialog" aria-modal="true" aria-labelledby="collector-title" onMouseDown={event => event.stopPropagation()}><div className="collector-heading"><div><small>CHROME COLLECTOR</small><h2 id="collector-title">采集开发页面属性</h2></div><IconButton label="关闭" onClick={onClose}>×</IconButton></div><div className="collector-method"><b>推荐 · Chrome 扩展</b><span>适用于无法修改源码的线上网站</span></div><div className="collector-steps"><span>1</span><p>打开 <code>chrome://extensions</code>，开启“开发者模式”，点击“加载已解压的扩展程序”，选择项目中的 <code>chrome-extension</code> 文件夹。</p></div><div className="collector-steps"><span>2</span><p>复制会话 ID，在待检查网页中打开 TARS 扩展，粘贴后点击“采集当前页面”。</p></div><div className="collector-code collector-session"><code>{id}</code><Button onClick={() => onCopy(id)}>复制会话 ID</Button></div><div className="collector-steps"><span>3</span><p>采集成功后返回这里，点击“检查连接并匹配”。请让网页视口宽度与当前设计稿一致。</p></div><details className="collector-fallback"><summary>可编辑源码？使用页面脚本方式</summary><div className="collector-code"><code>{snippet}</code><Button onClick={() => onCopy(snippet)}>复制脚本</Button></div></details>{error && <p className="collector-error">{error}</p>}<div className="collector-notice">扩展仅在你点击时读取当前标签页的可见文字样式和尺寸，不申请 Cookie、历史记录或网络监听权限。</div><div className="dialog-actions"><Button variant="ghost" onClick={onClose}>取消</Button><Button variant="primary" disabled={loading || !id} onClick={onCheck}>{loading ? "正在匹配…" : "检查连接并匹配"}</Button></div></div></div>;
}
