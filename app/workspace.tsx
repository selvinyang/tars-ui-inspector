...
function Storefront({ design = false, pageId }: { design?: boolean; pageId: string }) {
  const names: Record<string, string> = { home: "探索下一程", products: "城市出行系列", detail: "Ninebot MAX G2", cart: "你的购物车", checkout: "安全结算" };
  return <div className={`storefront ${design ? "storefront--design" : ""}`}>
    <header><div className="store-logo">SEGWAY</div><nav>电动滑板车　电动自行车　配件</nav><div className="store-actions">搜索　账户　购物车 2</div></header>
    <section className="store-hero"><div><small>BUILT FOR THE CITY</small><h1>{names[pageId]}</h1><p>更远续航，更稳操控。为每一段日常旅程而设计。</p><button>立即探索</button></div><div className="scooter-art"><span></span><i></i></div></section>
    <section className="store-categories"><b>通勤优选</b><span>城市系列</span><span>长续航</span><span>越野系列</span><span>智能配件</span></section>
    <section className="store-products">
      {["Ninebot MAX G2", "Segway E2 Plus", "Ninebot F2 Pro", "Segway GT2"].map((name, index) => <article key={name}><div className={`product-art art-${index}`}><i></i><span></span></div><small>{index % 2 ? "CITY ESSENTIAL" : "BEST SELLER"}</small><h3>{name}</h3><p>智能牵引力控制 · 双重减震</p><strong>¥{[6499, 2499, 4299, 18999][index].toLocaleString()}</strong></article>)}
    </section>
  </div>;
}
...
function DesignView({ asset, pageId, demo }: { asset?: DesignAsset; pageId: string; demo: boolean }) {
  return asset ? <ImageSurface asset={asset} label={`参考设计稿 ${asset.name}`} className="uploaded-design" /> : demo ? <Storefront pageId={pageId} design /> : <CanvasEmpty kind="design" />;
}

function ActualView({ asset, pageId, demo }: { asset?: DesignAsset; pageId: string; demo: boolean }) { return asset ? <ImageSurface asset={asset} label={`实际页面 ${asset.name}`} /> : demo ? <Storefront pageId={pageId} /> : <CanvasEmpty kind="actual" />; }

function CanvasContent({ mode, pageId, opacity, design, actual, alignment, zoom, onScore, onRegions }: { mode: CompareMode; pageId: string; opacity: number; design?: DesignAsset; actual?: DesignAsset; alignment: Alignment; zoom: number; onScore: (score: number) => void; onRegions: (regions: DifferenceRegion[]) => void }) {
  const demo = initialPages.some(page => page.id === pageId);
  if (!demo && !actual && !design) return <CanvasEmpty />;
  if (mode === "actual") return <ActualView asset={actual} pageId={pageId} demo={demo} />;
  if (mode === "design") return <DesignView asset={design} pageId={pageId} demo={demo} />;
  if (mode === "split") return <div className="split-view"><div><label>设计稿</label><DesignView asset={design} pageId={pageId} demo={demo} /></div><div><label>实际页面</label><ActualView asset={actual} pageId={pageId} demo={demo} /></div></div>;
  ...
  return <div className={`layer-view ${mode === "diff" ? "layer-view--diff" : ""}`}><ActualView asset={actual} pageId={pageId} demo={demo} />{(design || demo) && <div className="design-layer" style={{ opacity: mode === "diff" ? .72 : opacity / 100, transform: `translate(${alignment.x}px, ${alignment.y}px) scale(${alignment.scale})`, transformOrigin: "top left" }}><DesignView asset={design} pageId={pageId} demo={demo} /></div>}{mode === "diff" && <div className="diff-note">上传两张图片后启用真实差异</div>}</div>;
}
...
export function InspectorWorkspace() {
  ...
  const [pageId, setPageId] = useState("products");
  ...
  useEffect(() => { const raw = localStorage.getItem("tars-inspector-v1"); queueMicrotask(() => { if (raw) { try { const v = JSON.parse(raw); const savedPages = Array.isArray(v.pages) && v.pages.length ? v.pages as PageItem[] : initialPages; setProjectPages(savedPages); if (Array.isArray(v.issues)) setIssues(v.issues.map((issue: Issue) => issue.deviceId === "desktop-1280" ? { ...issue, deviceId: "small-1200" } : issue)); if (v.pageId) setPageId(savedPages.some(page => page.id === v.pageId) ? v.pageId : savedPages[0].id); if (v.deviceId) setDeviceId(v.deviceId === "desktop-1280" ? "small-1200" : devices.some(d => d.id === v.deviceId) ? v.deviceId : "small-1200"); if (v.alignments && typeof v.alignments === "object") setAlignments(v.alignments); } catch {} } storageReady.current = true; }); loadImages().then(items => { const nextDesigns: Record<string, DesignAsset> = {}; const nextActuals: Record<string, DesignAsset> = {}; items.forEach(item => { (item.kind === "design" ? nextDesigns : nextActuals)[item.pageId] = item.asset; }); setDesigns(nextDesigns); setActuals(nextActuals); }).catch(() => showToast("本地图片库读取失败")); }, []);
  ...
  useEffect(() => { if (!storageReady.current) return; localStorage.setItem("tars-inspector-v1", JSON.stringify({ pages: projectPages, issues, pageId, deviceId, alignments })); }, [projectPages, issues, pageId, deviceId, alignments]);
  ...
  function exportJson() { download("tars-ui-issues.json", JSON.stringify({ project: "Segway Store UI走查", exportedAt: new Date().toISOString(), issues: exportRows(visibleIssues) }, null, 2), "application/json;charset=utf-8"); }
      ...
      <div className={`canvas-viewport ${panning ? "is-panning" : ""} ${draggingFile ? "is-file-dragging" : ""}`} onPointerDown={startPan} onPointerMove={movePan} onPointerUp={() => setPanning(false)} onDragEnter={e => { e.preventDefault(); setDraggingFile(true); }} onDragOver={e => e.preventDefault()} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setDraggingFile(false); }} onDrop={handleDrop}>
        <div className="canvas-stage-bg" ref={canvasStage}>
          ...
          <div className="frame-slot" style={{ width: device.width * zoom / 100, height: canvasHeight * zoom / 100 }}><div className="device-frame" style={{ width: device.width, height: canvasHeight, transform: `translate(${position.x}px, ${position.y}px) scale(${zoom / 100})` }} onPointerDown={addMarker} data-testid="inspection-canvas">
              <CanvasContent mode={mode} pageId={pageId} opacity={opacity} design={designs[pageId]} actual={actuals[pageId]} alignment={alignment} zoom={zoom} onScore={setDiffScore} onRegions={setDiffRegions} />
